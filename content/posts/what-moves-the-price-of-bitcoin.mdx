...
takeaways:
  - "Bitcoin has no cash flows, so its price is set by how much speculative capital exists rather than by any intrinsic valuation."
  - "Global liquidity conditions are the largest single driver, which is why Bitcoin often moves with technology stocks despite no fundamental link."
  - "Leverage produces most violent moves: liquidations force selling, which pushes the price lower, which triggers more liquidations, with no news required."
faq:
