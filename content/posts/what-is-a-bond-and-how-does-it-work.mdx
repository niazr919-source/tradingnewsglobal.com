...
takeaways:
  - "A bond is a fixed schedule of payments, so when interest rates change the only thing that can adjust is the price."
  - "Duration measures price sensitivity: a bond with duration of 8 loses roughly 8% of its value if yields rise one percentage point."
  - "A bond fund never matures, so unlike an individual bond there is no date at which your capital is guaranteed back."
faq:
